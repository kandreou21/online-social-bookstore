...
public class CompositeRecommendationStrategyTest {
	@Autowired
	...
	void testRecommend() {
		...
		recomFormData.setRecommendationStrategy("composite");
		recomFormData.setAuthors(List.of(author));
		recomFormData.setCategories(List.of(category));
		
		List<BookFormData> books = compositeRecommendationStrategy.recommend("user", recomFormData);
